...
ui <- dashboardPage(
  ...
  dashboardSidebar(
    ...
    numericInput("selected_NG_margin", "Sand-gravel fraction margin (0-1)", 0.5, step = 0.05)
   #  
   #  br(),
   #      
  ...
  dashboardBody(
    ...
    fluidRow(
      box(width = 2,
          ...
          status = "primary",
             # textOutput("climate"),
      plotOutput("log", width="20vh", height="80vh"),
      # tableOutput("table")
      ),
      box(width = 5, 
          ...
          status = "primary",
          # textOutput("climate"),
          plotOutput("validating_plots", width="auto", height="80vh"),
          # tableOutput("table")
      )
    )
...
#### LOG GEN FUNCTION --------------------------------  
  ...
  plots_generator <- function(start_seed, iteration_number, include_logs = FALSE, include_validating_plots = FALSE,
    ...
    generated_log <- reactive({plots_generator(
    ...
    validating_plots <- reactive({plots_generator(
      start_seed <- input$start_seed,
      iteration_number = 1,
      input_bedding_data = bed_data_dummy,
      input_transition_data = bed_transition_data_dummy,
